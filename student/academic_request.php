<?php
    ...
    if (!isset($_SESSION["userId"])) {
        ...
        $facilitators = array();
        $sql = "SELECT facilitator_id, fname, lname FROM facilitator";
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
            
            // $facilitators = mysqli_fetch_array($result);
            while ($row = $result->fetch_assoc()) {
                $facilitators[] = ["name"=>$row["fname"] . ' ' . $row["lname"],"id"=>$row["facilitator_id"]];
            }
            } else {
        echo "No facilitators found in the database.";
        }
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $facilitatorId = $_POST["facilitator"];


            // Prepare and execute an SQL query to insert the request into the database
            $sql = "INSERT INTO academic_request (request_text, date_submitted, student_id, facilitator_id) VALUES (?, NOW(),?,?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sii", $requestText, $_SESSION["userId"], $facilitatorId);
...
<body>
    ...
    <!-- Error handling ~ displaying to user -->
        ...
        <form method="post">
            ...
            <div class="form-group">
                <label for="password">Choose a Facilitator</label>
                <select class="form-control" id="facilitator" name="facilitator" required>
                    <?php
